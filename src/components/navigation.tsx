...
export default function Navigation() {
  ...
  useGSAP(() => {
    ...
    introTL.fromTo(
      ".navigation",
      { opacity: 0, y: -60 },
      { opacity: 1, y: 0, ease: "back.out(2)" }
    );
  });

  return (
    <>
      <nav className="navigation opacity-0 fixed h-24 w-full flex justify-center items-center navbar-effect z-10 pb-6 ">
        <div className=" grid grid-cols-[70%_30%] lg:grid-cols-[30%_40%_30%] max-w-screen-2xl w-full h-fit items-center">
          ...
          <div className="hidden lg:grid lg:grid-cols-3 text-base text-black lg:items-center font-jakarta ">
            <Link
              href="/#why-email-pro"
              className="ml-auto bg-[#E4DED3]/90 underline min-w-fit p-1 rounded-xl"
            >
              Why Email Pro?
            </Link>
            <Link
              href="/#pricing"
              className="mx-auto bg-[#E4DED3]/90 underline min-w-fit p-1 rounded-xl "
            >
              Pricing
            </Link>
            <Link
              href="/#faqs"
              className="mr-auto bg-[#E4DED3]/90 underline min-w-fit p-1 rounded-xl "
            >
              FAQs
            </Link>
          </div>
          <div className="flex items-center justify-end pr-4 lg:pr-8 h-full">
            ...
            <button
              className="text-black text-4xl px-2 block lg:hidden drop-shadow-[0px_0px_15px_rgba(255,255,255)] h-fit bg-[#E4DED3]/90 w-fit rounded-xl p-1"
              onClick={toggleMenu}
      ...
      {/* Responsive Menu */}
      <div
        className={`fixed inset-0 bg-[#E4DED3] z-50 lg:hidden transition-opacity duration-300 ease-in-out ${
          isMenuOpen
